...
filename clip clear;";
%mend markdsn;</pre></li><li>%vvalue, automagically copies variable value.<pre class="prettyPrint">%macro vvalue();
gsubmit '
dm "wcopy";
...
filename clip clear;';
%mend vvalue;<</pre></li></ol><p>Prerequisites:</p><ol><li>Store the macros in an autocall library</li><li>Define a global macro variable named INCREMENT with initial value 0 in setup program</li><li>In command line type below commands to assign keys to evoke these macros<pre class="prettyPrint">keydef 'F9' '%makedsn'
